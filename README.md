...
`.claude/skills/hardware-truth/SKILL.md` —— 把本次移植中用翻车换来的取证纪律
沉淀成可复用的 Skill：读数可信度自证、一次一变量、实验必须有对照、结论变了
先改标题，以及"已取证／仅构建级／未验证"三态输出规范。

不是为提交而写的模板：里面每条规矩后面都挂着本仓真实的翻车案例（悬空脚被读成
"正在充电"、`fifo_status` 字段撒谎、误判芯片支持经典蓝牙），PORTING_NOTES 各章
可逐条对上。
