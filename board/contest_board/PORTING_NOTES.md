...
但第一次采到的中断计数是 0,我据此写下"链路层没拿到中断、
广播很可能没真发"。错的——**采样点在开发射之前**,那时候链路层
本来就无事可调度。挪到开发射之后再采:0 → 15 → 108 → 203,
每 2 秒约 95 次,合每秒 47 次。100ms 广播间隔、三信道,收发中断
加起来正是这个量级。寄存器也全部对上:BTSP 已上电、BTDM/XVR
时钟已开、睡眠钟停在 26M 分频的出厂档位。

这同时解释了此前每条 HCI 命令要等约 5 秒才有应答:广播没开时
没有任何东西唤醒控制器线程,只能靠下一条命令提交时顺带跑一轮。

### 现状与余量

BLE 广播在 NuttX 上跑通了。往下还有三层:GATT 服务(可挂闭源
