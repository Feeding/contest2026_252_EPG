...
闭源 BLE 协议栈在 NuttX 上跑通了,射频经校准后**收发都工作**:
冷启动扫描稳定听到 400+ 条周边广播,自身以 `openvela-EPG` 广播。
SARADC 驱动、射频校准、空中收发全部就位。

余量按价值排序:**修 PHY 表那两个电源/时钟槽位**(恢复厂商的
校准顺序,也是最后一处"知其然不知其所以然");出厂标定数据
读取(需要 flash 读驱动,目前发射功率用板级默认表,不是出厂
标定值);然后才谈得上 GATT 服务(可挂闭源
