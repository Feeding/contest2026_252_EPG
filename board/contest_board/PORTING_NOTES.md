...
规范命令直接经 VHCI 灌进去:复位、广播参数、广播数据(设备名
`openvela-EPG`)、开发射。全部返回 status 00。

第一次采到的中断计数是 0,我据此写下"链路层没拿到中断"。错的
——**采样点在开发射之前**,那时链路层本就无事可调度。挪到之后
再采:0 → 15 → 108 → 203,每秒约 47 次,正合 100ms 间隔三信道
广播的开销。寄存器也全对上:BTSP 已上电、BTDM/XVR 时钟已开、
睡眠钟停在 26M 分频档。于是我写下"射频在工作"。

**也错了。** 用户拿手机扫不到。中断在涨只证明链路层在按时调度
事件,不证明真有射频能量出去——这是两回事,我把前者当成了后者。

真正的判据是让板子**自己去听**:周围一定有 BLE 设备在广播,
收得到就说明收发通路和天线是活的。板子扫 10 秒,一个都听不到。
结论推翻:射频是聋的,也是哑的。

病根是**射频从来没有校准过**。厂商在纯蓝牙路径里,`app_ble_init()`
之前必跑 `bk_cal_if_init()`,我们的分级流程跳过了它——控制器
日志里那句 `cali_ready_status:0x0` 一直在说这件事,我一直当它
是"未标定但能用"的提示。

补上这一步之后,它在闭源库内部直接 UsageFault:

```
bk7258_bt_cal_init → bk_cal_if_init → calibration_init → calibration_main
```

UsageFault 是除零的典型signature,而校准正要靠 SARADC 读发射
功率(TSSI)、芯片温度、电源电压来算整定值——我们 PHY 表里那些
ADC 回调全是返回失败的 stub。拿不到测量值的算术崩掉,合理。

所以校准被移出主路径(留作 `face BT 7`,存档这个失败),**SARADC
驱动是下一块必须啃的硬骨头**,不是调参能绕过去的。侦察报告开篇
就把"没有 flash 标定数据 + 没有 ADC"列为最大风险,一语中的。

### 现状与余量

闭源 BLE 协议栈在 NuttX 上跑起来了,但**射频还没真正上天**:
链接、三张表、控制器启动到 `enter normal mode`、HCI 命令全通、
链路层按时调度事件——这些都实测通过;差的是能发出去的射频,
卡在校准,而校准卡在 SARADC。

往下的路,按依赖顺序:**SARADC 驱动**(校准的前置,也是温补和
电压监测的前置)→ 射频校准跑通 → 空中验证。之后才谈得上
GATT 服务(可挂闭源
host 的 API,或给 NuttX 原生蓝牙栈写一个 `bt_driver_s` 传输
shim——后者约 200~300 行,厂商 SDK 里就有一个 Zephyr host 挂
VHCI 的现成范例可抄);射频校准(需要 flash 读取器 + SARADC);
以及 Wi-Fi——同一套打法但表大得多(208 项),外加 pbuf 语义和
lwIP 的取舍,是数量级更大的工程。
