...
**两个硬件发现**:
1. **GDMA(0x45020000)对 CPU0 读写皆总线炸机**(疑似保护属性门控,
   `xd` 一读就重启)。SDK 的"必须 DMA"路径被 CPU 轮询替代——码率
   控制把帧封在 35KB,紧轮询搬运能力数十 MB/s,余量 50 倍。
2. **panic 里的 "irq: 5" 是 BusFault 向量,不是外设中断号**。
   第一次看到时追着 PWM0 跑了一圈。

**策略**:传感器先出流、AEC 收敛 400ms 后再武装编码器——首个 EOF
即成熟帧,完全绕开多帧 DMA 记账。实测两张(14,983 / 15,315 字节),
JFIF 结构完整(SOI..EOI,SOF=640×480),经串口 hexdump 捞回主机
解码成功。
...
- GDMA 保护门控之谜(CPU0 读写皆 BusFault,已绕过);
...
| 摄像头取流 | snap 两帧 JFIF 校验 + 主机解码 | 阶段二通过 |
